...
fn is_prefix(op: &TokenKind) -> bool {
    matches!(op, TokenKind::Bang | TokenKind::Minus | TokenKind::StringCast | TokenKind::ObjectCast)
}

fn prefix_binding_power(op: &TokenKind) -> u8 {
    match op {
        TokenKind::StringCast | TokenKind::ObjectCast => 101,
        TokenKind::Minus => 100,
        TokenKind::Bang => 99,
        _ => unreachable!()
    }
...
fn prefix(op: &TokenKind, rhs: Expression) -> Expression {
    match op {
        ...
        TokenKind::StringCast | TokenKind::ObjectCast => Expression::Cast { kind: op.into(), value: Box::new(rhs) },
        _ => unreachable!()
    }
}
...
fn infix_binding_power(t: &TokenKind) -> Option<(u8, u8)> {
    Some(match t {
        ...
        TokenKind::Equals | TokenKind::PlusEquals | TokenKind::DotEquals | TokenKind::CoalesceEqual | TokenKind::AsteriskEqual => (0, 1),
        _ => return None,
    })
}
